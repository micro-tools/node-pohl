...
class Pohl extends EventEmitter {
    ...
    constructor(config = { topic: "pohl", log: false, timeout: 1000, cache: { redis: null }, circuitCheck: 2000, timeoutThreshold: 5}){
        ...
        this.lock = new Redlock(
            [ new ioredis(config.cache.redis)],
            {
                driftFactor: 0.01,
                retryCount: 0,
                retryDelay: 200
    ...
    _runCircuitBreakerTimeoutCheck(circuitCheck = 2000, timeoutThreshold = 10){
        this._cbIntv = setInterval(() => {
            ...
            if(this._timeoutCount > timeoutThreshold){
                LOG.info("circuit breaker tripped, circuit open.");
    ...
    _setupTaskSender(done = null){
        ...
        this.incQueue.autoSubscribeChannel((message) => {
            ...
            LOG.debug("receiver returned a task: " + message);
    ...
    _onTaskReturned(_message){
        ...
        if(!message || typeof message !== "object"){
            LOG.error("[s] received bad message format: " + _message);
            return;
        }
        ...
        if(message.i === undefined ||
            message.p === undefined ||
            message.o === undefined){
            LOG.error("[s] received bad message content: " + _message);
            return;
        }
        ...
        if(!this.stack[message.i]){
            LOG.debug(message.i + " is not a task send from this sender, or it has timed out already.");
            return;
        } else {
            LOG.debug(message.i + " has a task send from this sender.");
        }
    ...
    _onTaskReceive(_message, callback){
        ...
        this.lock.lock(lockId, this.lockTTL).then(lock => {
            ...
            LOG.debug("received task and got lock for id: " + message.i + " on topic: " + this.topic);
            ...
            if(err){
                LOG.debug("did not get lock or failed during error: " + err);
            }
        });
    }
    ...
    setupTaskReceiver(taskEvent, done = null){
        ...
        this.outQueue.autoSubscribeChannel((message) => {
            ...
            if(this._paused){
                LOG.debug("received message but receiver is paused.");
                return;
            }
    ...
    sendTask(payload, callback = null){
        ...
        if(!callback){
            LOG.info("sendTask is missing callback, i hope you sure know what you are doing.");
            callback = FNULL;
        }
        ...
        LOG.debug("sending task with id: " + taskId + " on topic: " + this.topic);
