...
class RedisMessage {
    ...
    _setupPublish(){
        ...
        MLOG.info("publishing ready for " + this.channel);
    }
    ...
    _setupSubscribe(){
        ...
        MLOG.info("subscription ready for " + this.channel);
    }

    initRedisConfig(config){

        if(!config || !config.cache || !config.cache.redis){
            MLOG.error("Missing redis conf: " + JSON.stringify(config.cache));
    ...
    autoSubscribeChannel(triggerEvent, callback = null){
        ...
        if(!triggerEvent || typeof triggerEvent !== "function"){
            MLOG.error("TriggerEvent should be a valid function, for redis messages to be received.");
            return;
        }
        ...
        this.sub.subscribe(this.channel, (err, count) => {
            ...
            MLOG.info("Subscribed to redis channel: " + this.channel);
            ...
            this.sub.on("message", (channel, message) => {
                ...
                switch(channel) {
                    ...
                    case this.channel:
                        ...
                        MLOG.debug("received message on channel: " + channel);
    ...
    publishOnChannel(message){
        ...
        MLOG.debug("publishing redis message on channel:" + this.channel);
